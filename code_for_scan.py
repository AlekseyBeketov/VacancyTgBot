...
def employee_pick(message):
    ...
    if "Далее" not in message.text:
        if emp==0:
            bot.send_message(message.chat.id,"Отлично, если ты хочешь, ты можешь выбрать или убрать еще несколько областей\n\n"
                                             "Чтобы подтвердить выбранные✅ области, нажми кнопку ДАЛЕЕ",reply_markup=markup)
        if emp==1:
            bot.send_message(message.chat.id,"Отлично, если хотите, Вы можете выбрать или убрать еще несколько областей. "
