...
def startDbEmployee(message):
    ...
    cur = conn.cursor()
    cur.execute('create table if not exists users (id int primary key, name varchar(30), area_1 int, area_2 int, area_3 int, area_4 int, area_5 int, area_6 int, area_7 int, area_8 int, zanatost int, dlitelnost int)')
    cur.execute('create table if not exists workers (id int auto_increment primary key, id_emp int, name varchar(30), area_1 int, area_2 int, area_3 int, area_4 int, area_5 int, area_6 int, area_7 int, area_8 int, zanatost int, dlitelnost int,text varchar(5000), contacts varchar(500),date_of_create datetime)')
    conn.commit()
    cur.close()
    conn.close()
...
def employee_areas(message):
    ...
    markup.row(btn9)
    if emp==0:
        bot.send_message(message.chat.id, f'Хорошо, {emp_name}. Теперь давай выберем области, в которых ты хотел бы работать\n\n1. Веб-дизайн и разработка: создание уникальных дизайнов, верстка, адаптивный дизайн, UI/UX оптимизация;\n\n2. Копирайтинг и контент-маркетинг: написание продающих текстов, SEO оптимизация, создание контента для блогов и социальных сетей;\n\n3. Цифровой маркетинг: настройка рекламных кампаний, аналитика, SEO, SMM, контекстная реклама;\n\n4. Администрирование и поддержка сайтов: обновление контента, управление хостингом, обеспечение безопасности, техническая поддержка;\n\n5. Видеомонтаж и анимация: создание и монтаж видеороликов, анимация логотипов, спецэффекты;\n\n6. Графический дизайн: разработка логотипов, фирменного стиля, баннеров, иллюстраций;\n\n7. Переводы и локализация: письменный и устный перевод, адаптация контента под разные регионы;\n\n8. Программирование и разработка приложений', reply_markup=markup)
    if emp==1:
...
def employee_pick(message):
    ...
    if message.text == "Далее":
        ...
        if emp==0:
            ...
            cur.execute(
                    "insert into users (id, name, area_1, area_2, area_3, area_4, area_5, area_6, area_7, area_8, zanatost, dlitelnost) values ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')" % (
...
def employee_zanatost(message):
    ...
    markup.row(btn1, btn2, btn3)
    sent_message = bot.send_message(message.chat.id, 'Хорошо, теперь определимся с занятостью 🕒\n\nПолная занятость: работа 8 часов в день, 5 дней в неделю;\n\nЧастичная занятость: работа определенное количество часов в день или неделю, например, 4 часа в день или 20 часов в неделю;\n\nПроектная работа: работа на временной основе для выполнения конкретного проекта или задания',reply_markup=markup)
...
def contact_click(message):
    ...
    if emp==2:
        ...
        cur = conn.cursor()
        cur.execute(
            "insert into workers (id_emp, name, area_1, area_2, area_3, area_4, area_5, area_6, area_7, area_8, zanatost, dlitelnost, text, contacts,date_of_create) values ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')" % (
